...
class List:
    ...
    def clean(self):
        ...
        """
        # Your code here
        nondup = []  # to store first occurrences
        indexes = []  # indexes of duplicates
        i = 0
        while i < len(self._values):
            if self._values[i] not in nondup:
                nondup.append(self._values[i])
            else:
                indexes.append(i)
            i += 1

        for x in range(len(indexes) - 1, -1, -1):
            del self._values[indexes[x]]
        return
    ...
    def combine(self, source1, source2):
        ...
        while source1.is_empty() is False or source2.is_empty() is False:
            if source1.is_empty() is False:
                val = deepcopy(source1._values[0])
                self._values.append(val)
                del source1._values[0]
            if source2.is_empty() is False:
                val = deepcopy(source2._values[0])
                self._values.append(val)
    ...
    def insert(self, i, value):
        ...
        """
        # Your code here
        if i >= len(self._values):  # if i is more than len of list
            self._values.append(deepcopy(value))  # just appending
        else:  # the harder occurances
            if i < -(len(self._values)):  # if i is less than negative len of list
                i = 0  # same thing as prepending
            # if i is in the negative index of list
            elif i >= -(len(self._values)) and i < 0:
                i = len(self._values) + i
            # append a random value to the end and move everything
            # over to the right and overwrite index i with value
            #
            # this code runs regardless but i is changed to a more usable
            # index number if its not within the positive range of list
            self._values.append(None)  # appending an extra value for space
            w = len(self._values) - 1
            while w > i:
                # moving over all the values
                self._values[w] = self._values[w-1]
                w -= 1
            self._values[i] = deepcopy(value)
        return
    ...
    def intersection(self, source1, source2):
        ...
        """
        while source1.is_empty() is False or source2.is_empty() is False:
            if source1.is_empty() is False and deepcopy(source1._values[0]) not in self._values:
                val = deepcopy(source1._values[0])
                self._values.append(val)
                del source1._values[0]
            if source2.is_empty() is False and deepcopy(source2._values[0]) not in self._values:
                val = deepcopy(source2._values[0])
                self._values.append(val)
                del source2._values[0]
        """
        len1 = len(source1._values)
        len2 = len(source2._values)
        i = 0  # for source1
        # j = 0  # for source2
        while i < len1:
            if deepcopy(source1._values[i]) in source2._values and deepcopy(source1._values[i]) not in self._values:
    ...
    def split(self):
        ...
        # finding half-point index for even and odd lengths
        if length % 2 == 0:
            i = int(length / 2 - 1)
        else:
            i = length // 2
        target1._values = self._values[0:i+1]
        target2._values = self._values[i+1:]
        self._values = []
    ...
    def union(self, source1, source2):
        ...
        j = 0  # for source2
        x = 0
        if len1 < len2:  # for the bigger one
            lenbig = len2
        else:
            lenbig = len1
        while x < lenbig:
            if i < len1 and source1._values[i] not in self._values:
                self._values.append(deepcopy(source1._values[i]))
            if j < len2 and source2._values[i] not in self._values:
