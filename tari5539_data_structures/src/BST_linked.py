...
class BST:
    ...
    def parent(self, key):
        ...
        parent = None
        found = False
        # keep searching and iterate thru it
    ...
    def _parent_aux(self, node, key, parent):
        ...
        if node is None:
            ...
            # General Case #2 : Search the left
            value = self._parent_aux(node._left, key, node)  # go again
        elif key > node._value:
            # General Case # 3: Search the right
            value = self._parent_aux(node._right, key, node)  # go again
        elif parent is None:
