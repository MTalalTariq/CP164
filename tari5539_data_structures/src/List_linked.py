...
class List:
    ...
    def prepend(self, value):
        ...
        """
        # your code here
        node = _List_Node(deepcopy(value), self._front)
        self._front = node
        if self._count == 0:
            self._rear = node
        self._count += 1
    ...
    def insert(self, i, value):
        ...
        # if i is out of bounds
            ...
            # turn negative i to positive
            if i >= -len(self) and i < 0:
                i = int(len(self) + i)

                # print(i)

            behindnode = self._front
            indexcount = 0
            #i -= 1
            while behindnode._next is not None or indexcount <= i:

                if indexcount == i-1:
                    # print(behindnode._value)
                    behindnode._next = _List_Node(
                        deepcopy(value), behindnode._next)
                indexcount += 1
                behindnode = behindnode._next
            self._count += 1
    ...
    def remove(self, key):
        ...
        if len(self) > 0:
            ...
            if key != -1 and current is not None:
                if self._front == current and current._next is not None:
                    self._front = current._next
                    value = current._value
                    self._count -= 1
                elif self._front == current and current._next is None:
                    self._front = None
                    self._rear = None
                    value = current._value
                    self._count -= 1
                elif self._rear == current:
                    self._rear = previous
                    previous._next = None
                    value = current._value
                    self._count -= 1
    ...
    def __setitem__(self, i, value):
        ...
        while current is not None and index != i:
            index += 1
            current = current._next
        valueold = deepcopy(current._value)
...
# task6
    ...
    def reverse_r(self):
        ...
        current, previous = self.reverseaux(current, previous)
        """
        while current is not None:
            temp = current._next
            current._next = previous
...
# task6
    ...
    def reverseaux(self, current, previous):
        """
        ...
        if current is None:
            ...
            previous = current
            current = temp
            self.reverseaux(current, previous)
...
# task3
    ...
    def split_alt_r(self):
        ...
        target2 = List()
        left = True
        target1, target2 = self.splitaltaux(left, target1, target2)
        """
        while self._front is not None:
...
# task3
    def splitaltaux(self, left, target1, target2):
        """
...
# task1
    ...
    def _linear_search_r(self, key):
        ...
        previous, current, index = self.linearsearchaux(
            previous, current, index, key)
        """
        while current is not None and current._value != key:
            previous = current
            current = current._next
...
# task1
    ...
    def linearsearchaux(self, previous, current, index, key):
        """
...
# task4
    ...
    def intersection_r(self, source1, source2):
        ...
        # your code here
        source1_node = source1._front
        source1, source2, source1_node = self.intersectionaux(
            source1, source2, source1_node)
        """
        while source1_node is not None:
            value = source1_node._value
            _, current, _ = source2._linear_search(value)
...
# task4
    ...
    def intersectionaux(self, source1, source2, source1_node):
        """
        auxliliary for intersection_r
        """
        if source1_node is None:
            x = 1
        else:
            value = source1_node._value
            _, current, _ = source2._linear_search(value)
            ...
            source1, source2, source1_node = self.intersectionaux(
                source1, source2, source1_node)
        return source1, source2, source1_node
# task5
...
# task2
    ...
    def is_identical_r(self, target):
        ...
        if self._count != target._count:
            ...
            source_node = self.isidaux(source_node, target_node)
            """
            while source_node is not None and source_node._value == target_node._value:
                source_node = source_node._next
                target_node = target_node._next
    ...
    def isidaux(self, source_node, target_node):
        """
        ...
        if source_node is None:
            ...
            #print(source_node._value, target_node._value)
            source_node = "not none"
        else:
            # print(source_node._value)
            # print(target_node._value)
            source_node = source_node._next
            target_node = target_node._next
