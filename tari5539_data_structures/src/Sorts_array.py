    ...
    def radix_sort(a):
        ...
        if len(a) > 0:
            for i in range(len(a)-1, -1, -1):
                locat = a[i] % 10
                buck1s[locat].append(a[i])
        # print(buck1s)

        # reconstruct from BUCKET1s
        x = 0
        for i in buck1s:
            for j in i:
                a[x] = j
        ...
        if len(a) > 0:
            for i in range(len(a)-1, -1, -1):
                #recon[i] = recon[i] / 10
                locat = int(a[i] // 10)
                buck10s[locat].append(a[i])
        # print(buck10s)

        # reconstrcuted from BUCKET10s
        x = 0
        for i in buck10s:
...
# a10 t03
    ...
    def gnome_sort(a):
        ...
        if len(a) > 0:
            ...
            sorted = False
            while not sorted:
                if i == 0:  # 0 is always first
                    i += 1

                elif i > 0:
                    if a[i - 1] <= a[i]:  # left and front pot in order
                        i += 1  # move right, no swap
                    elif a[i - 1] > a[i]:  # left and front pot NOT in order
                        temp = a[i]
                        a[i] = a[i - 1]
                        a[i - 1] = temp  # swap
                        i -= 1  # move left
                if i >= len(a):  # checking if gnome is at end of list
                    sorted = True
        return
