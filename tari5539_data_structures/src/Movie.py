...
class Movie:
    ...
    def __init__(self, title, year, director, rating, genres):
        ...
        -------------------------------------------------------
        """
        assert year >= Movie.FIRST_YEAR, "Movie year must be >= {1888}".format(
            Movie.FIRST_YEAR)
        assert rating is None or Movie.MIN_RATING <= rating <= Movie.MAX_RATING, \
            "Movie ratings must be between {0} and {10}".format(
...
Genres:   {}""".format(self.title, self.year, self.director, self.rating, genres_list)
    ...
    def genres_string(self):
        ...
        if self.genres == None:
            ...
            string = Movie.GENRES[self.genres[0]]
    ...
    def genres_list_string(self):
        ...
        -------------------------------------------------------
        """
        string = str(self.genres[0])

        for i in range(1, len(self.genres)):
            string += f",{self.genres[i]}"
        # Your code here
