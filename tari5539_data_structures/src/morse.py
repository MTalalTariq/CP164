...
def encode_morse(bst, text):
    ...
    i = 0
    x = 0
    while i < len(text):  # for every word
        tempresult = ''
        while x < len(text[i]):  # for every letter
            # print(text[i][x])
            if text[i][x].isalpha():  # skips everything if not letter
                temptext = ByLetter(text[i][x].upper(), None)
                var = bst.retrieve(temptext)
                if tempresult != "":
                    tempresult += " "
                # print(var.code)
                tempresult = tempresult + var.code
                # print(str(var))
            x += 1
        if result != "":
            result += "\n"
...
def decode_morse(bst, code):
    ...
    while i < len(code):
        # if result != "":
        #    result += " "
        temptext = ByCode(None, code[i])
        var = bst.retrieve(temptext)

        result = result + var.letter
